...
namespace Tyuiu.KoryakinaAE.Sprint7.Project.V12.Lib
{
    public class DataService
    {
        ...
        public void SaveCsv(DataTable table, string path)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                var values = row.ItemArray
                    .Select(v => v == DBNull.Value || v == null ? "" : v.ToString().Replace(";", ","));
        ...
        public (int Count, double Min, double Max, double Average) GetColumnStatistics(DataTable table, string columnName)
        {
            if (table == null || !table.Columns.Contains(columnName))
                return (0, 0, 0, 0);

            var values = table.AsEnumerable()
                .Select(r => double.TryParse(r[columnName]?.ToString(), out var v) ? (double?)v : null)
                .Where(v => v.HasValue)
                .Select(v => v.Value)
                .ToList();
            ...
            return (values.Count, values.Min(), values.Max(), Math.Round(values.Average(), 2));
        }
